  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (restoreCustomerSession.fulfilled.match(result)) {
        ...
        setError("Account created but failed to restore session. Please try signing in.");
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md space-y-6 rounded-2xl border bg-card p-6 shadow-sm sm:p-8">
        <div>
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">
            Sign up to get started with Fibio Wholesale.
          </p>
        </div>
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            ...
            <TabsContent value="phone">
              <form onSubmit={handleSendOtp} className="space-y-4">
                <div>
                  <label htmlFor="signup-phone" className="block text-xs font-semibold text-foreground mb-1.5">
                    Mobile Number *
                  </label>
            ...
            <TabsContent value="email">
              <form onSubmit={handleSendOtp} className="space-y-4">
                <div>
                  ...
                  />
                  <p className="mt-1 text-[11px] text-muted-foreground">
                    We will send a 6-digit OTP code to verify your email address.
                  </p>
                </div>
